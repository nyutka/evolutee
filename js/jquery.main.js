...
// initTabHatihgt();
function initTabHatihgt() {
	var win = jQuery(window);
	jQuery('.table-section').each(function() {
		var holder = jQuery(this);
		var table = holder.find('table');
		var table1TR = table.eq(0).find('tr');
		var table2TR = table.eq(1).find('tr');
		var arr = [];
		function setHeight() {
			table1TR.each(function(i) {
				var arr = [];
				table1TR.eq(i).css({height:''});
				table2TR.eq(i).css({height:''});
				arr.push(table1TR.eq(i).outerHeight());
				arr.push(table2TR.eq(i).outerHeight());
				var max = Math.max.apply(null, arr);
				
				table1TR.eq(i).css({height:max});
				table2TR.eq(i).css({height:max});
			})
		}
